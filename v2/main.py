...
class TimeModel:
    def __init__(self, period, day_type, time_left) -> None:
        self.period = period
...
class Scheduler:
    ...
    def get_even_day_period_time(self, date):
        for times in EvenDayPeriodTimes:
            if date.time() < times.value:
                ...
                print(f"{times.value} is not less than {date.time()}")
        return None
    ...
    def get_info(self, date):
        ...
        time_left = self.get_time_left(date)
        ...
        return TimeModel(period, day_type, time_left)
...
# Test Monday, December 1st, 2020 at 2:00PM
print(f"It is a {s.get_day_type(datetime.datetime(year=2020, month=12, day=1, hour=14, minute=0))} day.")
print(f"There is {s.get_time_left(datetime.datetime(year=2020, month=12, day=1, hour=14, minute=0))} left in the period.")
# Test Monday, December 1st, 2020 at 9:45AM
print(s.get_info(datetime.datetime(year=2020, month=12, day=1, hour=9, minute=45)).time_left)